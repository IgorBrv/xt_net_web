...
{   // Предложите способ хранения информации о форматировании текста надписи и напишите программу, которая позволяет устанавливать и изменять начертание.
	...
	{  
		...
		{   // !!!!! Ниже приведено два решения, с использованием enum и словаря Dictionary. Мне больше нравится второе. 
			string str;
			byte num = 0;
			bool error = false;
			...
			while (true)
			{
				...
				if (byte.TryParse(str, out num) && num < 4) font.Set(num);
				else error = true;
	...
	{
		private List<byte> state = new List<byte> { 0 };  // Переменная, хранящая флаги состояния
