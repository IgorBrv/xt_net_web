...
{   // Объекты DAL слоя
	...
	{   // Объект DAO отвечающий за работу с сущностью наград
		...
		public DALAwardsJSON(IAwardsAssotiatonsDAO awardsAssotiatons, string path)
		{
			this.path = path;
		...
		public bool RemoveAward(Award award)
		{
			...
			if (data != null && dalJson.awardList.ContainsKey(award.id))
			{
				...
				if (data.emblemsList.ContainsKey(award.id))
				{
					try
					{
						data.emblemsList.Remove(award.id);
						File.Delete($"{path}{dalJson.emblemsList[award.id].Path.Substring(1)}");
					}
					catch (IOException)
					{
						//TODO
