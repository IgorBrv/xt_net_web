...
{	// Объекты DAL слоя
	...
	{   // Объект DAO отвечающий за работу с сущностью пользователей
		...
		public DALUsersJSON(IAwardsAssotiatonsDAO awardsAssotiatons, string path)
		{
			this.path = path;
		...
		public bool RemoveUser(User user)
		{
			...
			if (data != null && dalJson.userList.ContainsKey(user.id))
			{
				...
				if (data.emblemsList.ContainsKey(user.id))
				{
					try
					{
						data.emblemsList.Remove(user.id);
						File.Delete($"{path}{dalJson.emblemsList[user.id].Path.Substring(1)}");
					}
					catch (IOException)
					{
						//TODO
